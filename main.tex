...
\begin{equation}
    \begin{aligned}
    \eta^{n+1} =&\ 2w^n - w^{n-1}-k^2\omega_0^2w^n - 2u_{l_m}^n + u_{l_m}^{n-1} - \frac{Tk^2}{\rho A}\delta_{xx}u_{l_m}^n+\frac{EIk^2}{\rho A}\delta_{xxxx}u_{l_m}^n\\
...
\end{equation}
Recalling Eq. \eqref{eq:etaPlus}, and the derivative of an absolute value:
\begin{equation}
    |f|' = \frac{f}{|f|} \cdot f' =\text{sgn}(f)f'
\end{equation}
we can find its derivative:
\begin{equation}
    \Big[[f]_+\Big]' = \left[\frac{f+|f|}{2}\right]' = \frac{1+\text{sgn}(f)f'}{2}
\end{equation}
Now, using the quotient rule, $G'([\eta^{n+1}]^i)$ can be calculated:
\begin{equation}
    \begin{aligned}
    ...
    &= 1 - \left(\frac{k^2}{\rho Ah}+\frac{k^2}{M}\right)\left(\frac{K}{\alpha+1}\right)\left[\frac{[\eta^{n+1}]_+^{\alpha+1}-[\eta^{n-1}]_+^{\alpha+1}}{\eta^{n+1}-\eta^{n-1}}\right]'
    % &= 1 - \left(\frac{k^2}{\rho Ah}+\frac{k^2}{M}\right) \frac{K}{2(\alpha+1)}\left[\frac{(\eta^{n+1}+|\eta^{n+1}|)^{\alpha+1}-(\eta^{n-1}+|\eta^{n-1}|)^{\alpha+1}}{\eta^{n+1}-\eta^{n-1}}\right]'\\
    & \\
    &\begin{gathered}
        \text{Quotient rule: }\boxed{\left[\frac{f}{g}\right]' = \frac{f'g - fg'}{g^2}} \qquad f' = (\alpha+1)[\eta^{n+1}]_+^\alpha\left(\frac{1+\text{sgn}(\eta^{n+1})}{2}\right) \qquad g' = 1
    \end{gathered}\\
    & \\
    % & 1 - \left(\frac{k^2}{\rho Ah}+\frac{k^2}{M}\right) \frac{K}{2(\alpha+1)}\\
    & = 1 - \left(\frac{k^2}{\rho Ah}+\frac{k^2}{M}\right) \frac{K}{\alpha+1}\left(\frac{(\alpha+1)[\eta^{n+1}]_+^\alpha\left(\frac{1+\text{sgn}(\eta^{n+1})}{2}\right)\cdot(\eta^{n+1}-\eta^{n-1})-\Big([\eta^{n+1}]_+^{\alpha+1}-[\eta^{n-1}]_+^{\alpha+1}\Big)\cdot 1
    }{(\eta^{n+1}-\eta^{n-1})^2}\right)\\
    & = 1 - \left(\frac{k^2}{\rho Ah}+\frac{k^2}{M}\right) \frac{K}{\alpha+1}\left(\frac{(\alpha+1)[\eta^{n+1}]_+^\alpha\Big(1+\text{sgn}(\eta^{n+1})\Big)}{2(\eta^{n+1}-\eta^{n-1})}-\frac{[\eta^{n+1}]_+^{\alpha+1}-[\eta^{n-1}]_+^{\alpha+1}}{(\eta^{n+1}-\eta^{n-1})^2}\right)
\end{aligned}
\end{equation}
