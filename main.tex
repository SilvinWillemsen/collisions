...
\begin{subnumcases}{}
    \rho_\text{s} A \delta_{tt}u^n_l & $=T\delta_{xx}u^n_l - EI\delta_{xxxx}u^n_l - 2\sigma_{0, \text{s}}\delta_{t\cdot}u^n_l + 2\sigma_{1, \text{s}}\delta_{t-}\delta_{xx}u_l^n - J_\text{s}(l_\text{br})F_\alpha$ \\
